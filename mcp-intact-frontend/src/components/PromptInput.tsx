...
const predefinedPrompts = [
  ...
  { id: 'email', label: 'Send morning touchbase email', value: 'Send an email to Errol.F.Stewart@gmail.com with subject, Morning touchbase. The email should be, asking the receipient to join the Hackathon Demo' },
  { id: 'jeff', label: 'Jeff Crowe\'s claims', value: 'Email me the Claims details for Jeff Crowe\'s most recent claim' },
  { id: 'vehicles', label: 'Get me all Honda vehicles', value: 'Get me all Honda vehicles' },
  { id: 'custom', label: 'Custom prompt...', value: '' },
];

const PromptInput: React.FC<PromptInputProps> = ({ onSend, disabled, onPromptChange }) => {
...
const handlePromptChange = (promptId: string) => {
  ...
  setPrompt(selectedPrompt?.value || '');
    onPromptChange?.(); // Call the callback when prompt changes
};
